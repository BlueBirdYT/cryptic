...
async def on_ready():
     ...
     print('.......')
     print('created by Rohit and bluebird')
     client.loop.create_task(status_task())

def is_owner(ctx):
     return ctx.message.author.id in ["455322915471097857","535271559623868416"]



@client.event
async def on_member_join(member):
    role = discord.utils.get(member.server.roles, name='||POKEMON TRAINERS||')
...
async def userinfo(ctx, user: discord.Member=None):
    if user is None:
      await client.say('Please tag a user to get user information. Example- ``Nuserinfo @user``')
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    if discord.utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        await client.say("Use this command like ``Nrolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
    if value is None:
        await client.say("Use this command like ``Nrolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
...
async def warn(ctx, userName: discord.User=None,*, message:str=None):
    if userName is None:
      await client.say('Please tag a person to warn user. Example- ``Nwarn @user <reason>``')
      return
    else:
...
async def avatar(ctx, user: discord.Member=None):
    if user is None:
        ...
        embed.set_thumbnail(url='https://cdn.discordapp.com/avatars/487552378497662978/6efb04e0226c9f55eb4c785ac4488686.webp?size=1024')
...
async def unmute(ctx, member: discord.Member=None):
    if member is None:
      await client.say('Please specify member i.e. Mention a member to unmute. Example- ``Nunmute @user``')
    if ctx.message.author.bot:
      return
    else:
      if ctx.message.author.server_permissions.kick_members == False:
...
async def access(ctx, member: discord.Member=None):
    if member is None:
      await client.say("Please specify a member to give access to him. Example- ``Naccess @user``")
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def setnick(ctx, user: discord.Member=None, *, nickname=None):
    if user is None:
      await client.say('Please tag a person to change nickname. Example- ``Nsetnick @user <new nickname>``')
      return
    else:
...
async def ban(ctx,user:discord.Member=None):
    if user is None:
      await client.say('Please specify a member to ban. Example- ``Nban @user``')
...
async def rps(ctx, *, message=None):
    ...
    if message is None:
        await client.say('Use it like ``Nrps rock or scissors or paper`` anyone of them to make this command work properly')
...
async def kick(ctx,user:discord.Member):
    if user is None:
      await client.say('Please mention a member to kick. Example- ``Nkick @user``')
...
async def invite():
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.set_image(url="https://cdn.discordapp.com/avatars/487552378497662978/6efb04e0226c9f55eb4c785ac4488686.webp?size=1024")
    embed.add_field(name = 'Link to invite me', value='https://discordapp.com/api/oauth2/authorize?client_id=548415429006327830&permissions=8&scope=bot')
    embed.add_field(name = 'bot info', value='Cryptic is a simple bot easy and fun to use we will do updates and keep improving the bot')
    await client.say(embed=embed)
    await client.delete_message(ctx.message)
...
async def help2(ctx):
    ...
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information and Nhelp3 for more commands help')
@client.command(pass_context = True)
async def help3(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Main Creator: Rohit#2019", color=0x00ff00)
    embed.set_author(name=" Bot Owner ""Rohit#2019")
    embed.add_field(name="Co-owner: BlueBird ❄ Froakie collector#0440", value="He coded the bot")
    embed.set_image(url="https://cdn.discordapp.com/avatars/548415429006327830/e84a5558795deb0cb651dee2743a96b2.webp?size=1024")
    embed.add_field(name="bot info", value="Cryptic is a bot fun and simple to use we want people to have fun and we will keep improving it as much as we can")
...
async def server(ctx):
    await client.say('link to join support server https://discord.gg/XEST4pB')
